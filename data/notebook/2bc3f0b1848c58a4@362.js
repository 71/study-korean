import define1 from "./64e2af71a5929a03@397.js";
...
- Words with their definitions, part-of-speech, and translations can be downloaded from
  [krdict.korean.go.kr](https://krdict.korean.go.kr/download/downloadList) (account needed).
...
Since we don't need a lot of the data, we convert trim it down and convert it to something interesting below.`
)}

async function _xmlDicts(rawzipdict)
{
  const archive = await rawzipdict.zip(),
        xmlFiles = Object.fromEntries(
          await Promise.all(
            archive.filenames.map(async (x) => [x, await archive.file(x).xml()]),
          ),
        );
...
function _xmlEntries(xmlDicts){return(
Object.values(xmlDicts).flatMap((x) => [...x.querySelectorAll("LexicalResource Lexicon LexicalEntry")])
)}

function _entries(xmlEntries){return(
xmlEntries.flatMap((x) => {
  const
    sel = (x, selector) => x.querySelector(selector)?.getAttribute("val"),
    get = (x, att) => sel(x, `feat[att=${att}]`),
    getAll = (x, selector, f) => [...x.querySelectorAll(selector)].map(f);
  ...
  const
    pos = get(x, "partOfSpeech"),
    text = get(x, "writtenForm"),
    origin = get(x, "origin"),
    pronunciation = get(x, "pronunciation"),
    soundUrl = get(x, "sound"),
    ...
    meanings = getAll(x, "Sense", (y) => {
      ...
      for (const rel of y.querySelectorAll(`SenseRelation`)) {
      ...
      const
        definition = sel(x, `Sense[val="${y.getAttribute("val")}"] > feat[att=definition]`),
        ...
        translations = [...y.querySelectorAll("Equivalent")].flatMap((x) => {
      ...
      return { originalForms, abbreviations, synonyms, antonyms, related, definition, examples, translation: translations[0] };
    });

  return { lexicalUnit, pos, text, origin, pronunciation, soundUrl, meanings, relatedForms };
})
)}

function _words(entries){return(
entries.filter((x) => x.lexicalUnit === "단어")
)}

function _nonWords(entries){return(
entries.filter((x) => x.lexicalUnit !== "단어")
)}

function _dict(words)
{
  const byText = {};
...
function _12(download,dict){return(
download(async () => new Blob([JSON.stringify(dict)], { type: "application/json" }), "dict.json")
)}
...
async function _examples(examplesArchive,htmlToText)
{
  ...
  return rows.map(({ flds: row }) => {
    const fields = row.split("\x1f");
    let koRaw, en, enAlt, soundRaw, notesRaw;

    if (fields.length === 5) {
      [koRaw, en, enAlt, soundRaw, notesRaw] = fields;
    } else {
      // TODO: fix this case (with a different Anki deck).
      [koRaw, en, soundRaw, notesRaw] = fields;
    }

    const soundName = /^\[sound:(.+\.mp3)\]$/.exec(soundRaw)?.[1],
          sound = soundName != null ? media[soundName] : undefined,
          notes = htmlToText(notesRaw);
    let ko = koRaw.replace(/&nbsp;/g, " ");
...
md`We now tokenize the sentences so that we can find out which words are in which sentences,
courtesy of [Open Korean Text](https://github.com/open-korean-text/open-korean-text) (through
[Korean NLP](https://observablehq.com/@71/korean-nlp)).`
)}

function _examplesWithTokens(examples,tokenize){return(
examples.map((sentence) => {
  const tokens = tokenize(sentence.ko).map((t) => ({ ...t }));

  return { ...sentence, tokens };
})
)}

function _22(download,examplesWithTokens){return(
download(async () => new Blob([JSON.stringify(examplesWithTokens)], { type: "application/json" }), "examples.json")
)}
...
function _pos(examplesWithTokens){return(
new Set(examplesWithTokens.flatMap(s => s.tokens.map(t => t.pos)))
)}
...
function _wordsToExamples(mostCommonWordDefinitions,examplesWithTokens,interestingPos)
{
  ...
  for (let i = 0; i < examplesWithTokens.length; i++) {
    const sentence = examplesWithTokens[i];

    for (const token of sentence.tokens) {
      if (interestingPos.has(token.pos)) {
        const text = token.stem ?? token.text;
  ...
  return Object.fromEntries(
    Object.entries(words).map(([word, sentenceIndices]) => [word, [...sentenceIndices]]),
  );
}


function _27(download,wordsToExamples){return(
download(async () => new Blob([JSON.stringify(wordsToExamples)], { type: "application/json" }), "words_to_examples.json")
)}
...
function _30(dict,tokenize,deepdiveword)
{
  const
    cache = new Map(),
    ...
    for (const wordData of data) {
      for (const meaning of wordData.meanings) {
        ...
        result.definitions.push({
          ...
          get definitionWords() {
            return tokenize(definition).flatMap((token) => {
              const r = traverse(token.stem ?? token.text);
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("xmlDicts")).define("xmlDicts", ["rawzipdict"], _xmlDicts);
  main.variable(observer("xmlEntries")).define("xmlEntries", ["xmlDicts"], _xmlEntries);
  main.variable(observer("entries")).define("entries", ["xmlEntries"], _entries);
  main.variable(observer("words")).define("words", ["entries"], _words);
  main.variable(observer("nonWords")).define("nonWords", ["entries"], _nonWords);
  main.variable(observer("dict")).define("dict", ["words"], _dict);
  main.variable(observer()).define(["download","dict"], _12);
  ...
  main.variable(observer("examplesWithTokens")).define("examplesWithTokens", ["examples","tokenize"], _examplesWithTokens);
  main.variable(observer()).define(["download","examplesWithTokens"], _22);
  main.variable(observer()).define(["md"], _23);
  main.variable(observer("pos")).define("pos", ["examplesWithTokens"], _pos);
  main.variable(observer("interestingPos")).define("interestingPos", _interestingPos);
  main.variable(observer("wordsToExamples")).define("wordsToExamples", ["mostCommonWordDefinitions","examplesWithTokens","interestingPos"], _wordsToExamples);
  main.variable(observer()).define(["download","wordsToExamples"], _27);
  ...
  main.variable(observer()).define(["dict","tokenize","deepdiveword"], _30);
  main.variable(observer()).define(["md"], _31);
  main.variable(observer("htmlToText")).define("htmlToText", ["DOMParser"], _htmlToText);
  const child1 = runtime.module(define1);
  main.import("tokenize", child1);
